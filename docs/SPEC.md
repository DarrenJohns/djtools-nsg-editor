...
- CI/CD via GitHub Actions (`.github/workflows/deploy.yml`) on a hosted `ubuntu-latest` runner — auto-deploys on push to `main` using `@azure/static-web-apps-cli`. Deployment token is stored as a secret on the `production` GitHub Environment (branch-restricted to `main`).
...
├── .github/
│   ├── workflows/
│   │   ├── deploy.yml  # CI/CD: deploy to Azure SWA on push to main
